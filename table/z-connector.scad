...
module zConnector()
{
  rotate([0,0,270]) 
  {
    ...
    translate([12.5,-2.625,0]) cube([40,6.75,12], center=true);
    translate([0,-2.625,0]) cube([18,6.75,24.75], center=true);
    translate([8.5,0,0]) cube([6,12,24.75], center=true);
...
//#    mirror([0,0,0]) translate([y_bars_apart-21,0,0]) bearingHolder(top_hole=false, hold_bolt=false);
    translate([y_bars_apart-21,0,0]) yConnector();

    translate([34.5,0,15]) difference()
    {
      cube([8,12,10], center=true);
      translate([0,0,0.75]) rotate([90,0,0])  cylinder(h=connector_width, r=radius_bolt*1.1, center=true);
    }
